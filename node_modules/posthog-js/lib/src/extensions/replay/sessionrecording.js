...
import { CONSOLE_LOG_RECORDING_ENABLED_SERVER_SIDE, SESSION_RECORDING_CANVAS_RECORDING, SESSION_RECORDING_ENABLED_SERVER_SIDE, SESSION_RECORDING_IS_SAMPLED, SESSION_RECORDING_NETWORK_PAYLOAD_CAPTURE, SESSION_RECORDING_RECORDER_VERSION_SERVER_SIDE, } from '../../constants';
...
import { _isBoolean, _isFunction, _isNull, _isNullish, _isNumber, _isObject, _isString, _isUndefined, } from '../../utils/type-utils';
...
var SessionRecording = /** @class */ (function () {
    ...
    SessionRecording.prototype.makeSamplingDecision = function (sessionId) {
        ...
        if (!shouldSample) {
            logger.warn(LOGGER_PREFIX +
                " Sample rate (".concat(this._sampleRate, ") has determined that this sessionId (").concat(sessionId, ") will not be sent to the server."));
        }
    ...
    SessionRecording.prototype.afterDecideResponse = function (response) {
        ...
        if (this.instance.persistence) {
            this.instance.persistence.register((_a = {},
                ...
                _a[SESSION_RECORDING_CANVAS_RECORDING] = {
                    enabled: (_e = response.sessionRecording) === null || _e === void 0 ? void 0 : _e.recordCanvas,
                    fps: (_f = response.sessionRecording) === null || _f === void 0 ? void 0 : _f.canvasFps,
                    quality: (_g = response.sessionRecording) === null || _g === void 0 ? void 0 : _g.canvasQuality,
                },
                _a));
        }
        var receivedSampleRate = (_h = response.sessionRecording) === null || _h === void 0 ? void 0 : _h.sampleRate;
        this._sampleRate = _isNullish(receivedSampleRate) ? null : parseFloat(receivedSampleRate);
        var receivedMinimumDuration = (_j = response.sessionRecording) === null || _j === void 0 ? void 0 : _j.minimumDurationMilliseconds;
        this._minimumDuration = _isUndefined(receivedMinimumDuration) ? null : receivedMinimumDuration;
        this._linkedFlag = ((_k = response.sessionRecording) === null || _k === void 0 ? void 0 : _k.linkedFlag) || null;
        if ((_l = response.sessionRecording) === null || _l === void 0 ? void 0 : _l.endpoint) {
            this._endpoint = (_m = response.sessionRecording) === null || _m === void 0 ? void 0 : _m.endpoint;
        }
        if (_isNumber(this._sampleRate)) {
        ...
        if (!_isNullish(this._linkedFlag)) {
            var linkedFlag_1 = _isString(this._linkedFlag) ? this._linkedFlag : (_o = this._linkedFlag) === null || _o === void 0 ? void 0 : _o.flag;
            var linkedVariant_1 = _isString(this._linkedFlag) ? null : (_p = this._linkedFlag) === null || _p === void 0 ? void 0 : _p.variant;
            this.instance.onFeatureFlags(function (_flags, variants) {
                var flagIsPresent = _isObject(variants) && linkedFlag_1 in variants;
    ...
    SessionRecording.prototype._startCapture = function () {
        ...
        if (this.instance.__loaded_recorder_version !== this.recordingVersion) {
            loadScript(this.instance.requestRouter.endpointFor('assets', "/static/".concat(recorderJS, "?v=").concat(Config.LIB_VERSION)), function (err) {
                if (err) {
                    return logger.error(LOGGER_PREFIX + " could not load ".concat(recorderJS), err);
                }
    ...
    SessionRecording.prototype._processQueuedEvents = function () {
        ...
        if (this.queuedRRWebEvents.length) {
            ...
            var itemsToProcess_1 = __spreadArray([], __read(this.queuedRRWebEvents), false);
            this.queuedRRWebEvents = [];
            itemsToProcess_1.forEach(function (queuedRRWebEvent) {
                if (Date.now() - queuedRRWebEvent.enqueuedAt > TWO_SECONDS) {
                    _this._tryAddCustomEvent('rrwebQueueTimeout', {
                        enqueuedAt: queuedRRWebEvent.enqueuedAt,
                        attempt: queuedRRWebEvent.attempt,
                        queueLength: itemsToProcess_1.length,
                    });
                }
                else {
                    if (_this._tryRRWebMethod(queuedRRWebEvent)) {
                        _this._tryAddCustomEvent('rrwebQueueSuccess', {
                            enqueuedAt: queuedRRWebEvent.enqueuedAt,
                            attempt: queuedRRWebEvent.attempt,
                            queueLength: itemsToProcess_1.length,
                        });
