...
var safariCheck = function (ua, vendor) { return (vendor && _includes(vendor, APPLE)) || isSafari(ua); };
/**
 * This function detects which browser is running this script.
 * The order of the checks are important since many user agents
 * include keywords used in later checks.
 */
export var detectBrowser = function (user_agent, vendor, opera) {
    if (opera) {
        return OPERA;
    }
...
var versionRegexes = (_a = {},
    ...
    _a[CHROME] = [new RegExp(CHROME + '/' + BROWSER_VERSION_REGEX_SUFFIX)],
    ...
    _a[ANDROID_MOBILE] = [new RegExp('android\\s' + BROWSER_VERSION_REGEX_SUFFIX)],
 ...
 */
export var detectBrowserVersion = function (userAgent, vendor, opera) {
    var browser = detectBrowser(userAgent, vendor, opera);
...
export var detectOS = function (user_agent) {
    for (var i = 0; i < osMatchers.length; i++) {
        var _a = __read(osMatchers[i], 2), rgex = _a[0], resultOrFn = _a[1];
        var match = rgex.exec(user_agent);
        var result = match && (_isFunction(resultOrFn) ? resultOrFn(match, user_agent) : resultOrFn);
        if (result) {
            return result;
        }
    }
...
export var detectDevice = function (user_agent) {
...
export var detectDeviceType = function (user_agent) {
    var device = detectDevice(user_agent);
    if (device === IPAD ||
