...
import { _copyAndTruncateStrings, _each, _eachArray, _extend, _includes, _register_event, _safewrap_class, isCrossDomainCookie, isDistinctIdStringLike, } from './utils';
...
import { _isArray, _isEmptyObject, _isEmptyString, _isFunction, _isNumber, _isObject, _isString, _isUndefined, } from './utils/type-utils';
...
var PostHog = /** @class */ (function () {
    function PostHog() {
        ...
        this.compression = Compression.Base64; // Upgrades to gzip if decide response supports it
    ...
    PostHog.prototype._init = function (token, config, name) {
        ...
        if (config.segment) {
            config.segment.register(this.segmentIntegration()).then(function () {
                _this._loaded();
            });
        }
        else {
    ...
    PostHog.prototype._dom_loaded = function () {
        var _this = this;
        if (!this.has_opted_out_capturing()) {
            _eachArray(this.__request_queue, function (item) { return _this._send_retriable_request(item); });
        }
        this.__request_queue = [];
        this._start_queue_if_opted_in();
    };
    ...
    PostHog.prototype.capture = function (event_name, properties, options) {
        ...
        if (event_name === '$identify') {
            ...
            data['$set_once'] = options === null || options === void 0 ? void 0 : options['$set_once'];
        }
        ...
        if (!_isUndefined(options === null || options === void 0 ? void 0 : options.timestamp)) {
    ...
    PostHog.prototype._calculate_event_properties = function (event_name, event_properties) {
        ...
        if (_isArray(this.config.property_denylist) && _isArray(this.config.property_blacklist)) {
            ...
            var property_denylist = __spreadArray(__spreadArray([], __read(this.config.property_blacklist), false), __read(this.config.property_denylist), false);
    ...
    PostHog.prototype.get_session_replay_url = function (options) {
        ...
        var url = this.requestRouter.endpointFor('ui', '/replay/' + sessionId);
    ...
    PostHog.prototype.set_config = function (config) {
        ...
        if (_isObject(config)) {
            ...
            if (this.sessionRecording && !_isUndefined(config.disable_session_recording)) {
                var disable_session_recording_has_changed = oldConfig.disable_session_recording !== config.disable_session_recording;
                // if opting back in, this config might not have changed
                var try_enable_after_opt_in = !userOptedOut(this) && !config.disable_session_recording && !this.sessionRecording.started;
...
export function init_from_snippet() {
    ...
    if (snippetPostHog) {
        ...
        _each(snippetPostHog['_i'], function (item) {
