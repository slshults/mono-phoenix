defmodule MonoPhoenixV01Web.SearchwomenByPlay do
  @moduledoc """
  ...
  def get_all(query, play_id) do
    if is_nil(query) or String.trim(query) == "" do
      ...
      if length(words) <= 1 do
        ...
        case phrase_search(cleaned_query, play_id) do
          [] -> ranked_search(cleaned_query, play_id)
          results -> results
        end
      end
  ...
  defp phrase_search(cleaned_query, play_id) do
    from(m in "monologues",
      ...
      on: g.id == m.gender_id,
      where:
        m.play_id == ^play_id and (g.name == "Women" or g.name == "Both") and
      ...
      select: %{
        play: p.title, monologues: m.id, location: m.location,
  ...
  defp ranked_search(cleaned_query, play_id) do
    fts_query = normalize_for_fts(cleaned_query)
    if fts_query == "", do: [], else: do_ranked_search(fts_query, play_id)
  end

  defp do_ranked_search(fts_query, play_id) do
    from(m in "monologues",
      join: p in "plays",
      on: m.play_id == p.id,
      join: g in "genders",
      on: g.id == m.gender_id,
      where: m.play_id == ^play_id and (g.name == "Women" or g.name == "Both") and fragment(
        """
