defmodule MonoPhoenixV01Web.PlaysPageController do
  use MonoPhoenixV01Web, :controller
  import Ecto.Query, only: [from: 2]

  # Show lists of the plays
  @spec plays(Plug.Conn.t(), any) :: Plug.Conn.t()
  def plays(conn, _params) do
    query =
      from(p in "plays",
        group_by: [p.title],
        select: %{
...
#       @moduledoc """
#       Controller for plays
#       """
#
#       # Gets the index page for plays
...
#         session[:play] = nil
#         render(conn, "plays/index", title: title, plays: # plays, comedies: comedies, histories: histories, # tragedies: tragedies, scope: scope)
#       end
#
#       # Gets the index page for plays
#       get "/plays", play_index: true, cache: true
